...
def num_examples_validator(df: pd.DataFrame) -> Remediation:
    ...
    if n >= MIN_EXAMPLES:
        ...
        optional_suggestion = ". In general, we recommend having at least a few hundred examples. We've found that performance tends to linearly increase for every doubling of the number of examples"
    immediate_msg = f"\n- Your file contains {n} prompt-completion pairs{optional_suggestion}"
